...
The `:summary` mode shows a consolidated count of orphaned migrations instead of listing each one individually. This is useful for large teams where individual warnings can be noisy.

```ruby
MigrationGuard.configure do |config|
  config.warning_frequency = :summary
end
```

With `:summary` mode, instead of seeing individual warnings for each orphaned migration, you'll see a single consolidated message like:

```
⚠ MigrationGuard: 5 orphaned migrations detected (use `rails db:migration:status` for details)
```
